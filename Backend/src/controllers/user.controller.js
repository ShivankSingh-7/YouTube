...
const generateAccessAndRefreshTokens = async (userId) => {
  try {
    ...
    throw new ApiError(
      500,
      "something went qrong while generating refersh and access token"
    );
  }
};
...
const updateUserAvatar = asyncHandler(async (req, res) => {
  const avatartLoaclPath = req.file?.path;
...
const updateUserCoverImage = asyncHandler(async (req, res) => {
  ...
  if (!coverImage.url) {
    throw new ApiError(400, "Error while uploading coverImage on cloduinary");
  }
